...
- **Reviewer Assignment Round-Robin:** Integrated directly into the state machine inside `models.py`. The instant a submission completes the `draft` → `submitted` state transition, it fires a query to find the last assigned reviewer and rolls the pointer to the next active reviewer. To satisfy strict visibility requirements, reviewers can see *all* queue items, but the UI prominently displays an `Assignee` badge ("You" or the assigned reviewer's name) to make the round-robin logic highly visible and effective for queue management.
- **Strict Business Logic Validations:** We added heavy backend validation against edge-case evaluations. Merchants cannot submit without exactly 3 specific documents and a valid monthly volume. Reviewers cannot reject or request info without providing a written reason.
